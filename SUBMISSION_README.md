...
- 업로드 파일: `aicarracing_teamb_image.tar.gz.part_aa` ~ `part_ai` (90MB×8 + 4.5MB×1, **총 9개**)
- 이미지: `aicarracing-teamb:latest` (linux/amd64, CPU torch, 평가용 체크포인트 동봉)
- **복원·실행**(채점자):
  ```bash
  cat aicarracing_teamb_image.tar.gz.part_* > aicarracing_teamb_image.tar.gz
  gunzip -c aicarracing_teamb_image.tar.gz | docker load
  docker run --rm aicarracing-teamb:latest \
      python -m scripts.evaluate_agent_2action --model ./models/ppo_2action4/best_model.pth --episodes 100 --seed 42
  ```
- 빌드 재현: `bash build_docker.sh` (`Dockerfile`·`build_docker.sh` 포함). 자세한 설명은 `COMBINED_REPORT.pdf` §0.
